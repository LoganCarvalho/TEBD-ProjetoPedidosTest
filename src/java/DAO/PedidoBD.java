...
public class PedidoBD {
     private BDjdbc conexao; //outra classe deve ser criada
...
*/	
	public void AdicionarItemBD(Item item_,int pedido_id)throws Exception{
      ...
      "INSERT INTO PEDIDOS.ITEM (DESCRICAO,QUANTIDADE,VALOR,PEDIDO_ID) values (?,?,?,?)");
...
*/	
	public void AdicionarPedidoBD(Pedido pedido_)throws Exception{
	  PreparedStatement Stmt;
      Stmt = conexao.getConexao().prepareStatement(
      "INSERT INTO PEDIDOS.PEDIDO (ID,DESCRICAO) VALUES (?,?)");
      ...
      PreparedStatement Stmt;
      Stmt = conexao.getConexao().prepareStatement(
      "DELETE FROM PEDIDOS.PEDIDO WHERE  ID=?");
      ...
      ResultSet  rs;
      Stmt = conexao.getConexao().prepareStatement(
      "SELECT DESCRICAO FROM PEDIDOS.PEDIDO WHERE ID=?");
      Stmt.setInt(1,pedido_id.intValue());
      ...
      Stmt = conexao.getConexao().prepareStatement(
      "SELECT DESCRICAO,QUANTIDADE,VALOR FROM PEDIDOS.ITEM WHERE PEDIDO_ID=?");
      Stmt.setInt(1,pedido_id.intValue());
      rs = Stmt.executeQuery();
      while (rs.next()){
